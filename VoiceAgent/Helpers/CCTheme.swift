...
enum CC {
    ...
    enum Size {
        /// 底部那两条（说话条、控制栏）的高度。**一个常量，因为它们必须等高。**
        ///
        /// ⛔ 原来是两个数：说话条 64、控制栏 56，理由写的是
        /// 「主操作要比一般按钮大一号」。那条理由在**单独看说话条**时成立，
        /// 放进真机整屏就不成立了 —— Chris 2026-09-20：
        ///
        /// > 那个按钮得跟下边那一个是一样高的，你单把它一个人变高多难看呢。
        ///
        /// 他说得对：两条同宽、同圆角、上下紧挨着，**只有高度差 8pt** ——
        /// 那不读作「这条更重要」，读作「没对齐」。要强调主次有的是别的手段
        /// （染色、字重、位置），拿高度去表达反而先破坏了秩序感。
        ///
        /// **拆成两个常量本身就是那个 bug 的温床**：两个数没有任何机制保证相等，
        /// 谁调一下就悄悄劈叉了。合成一个，「必须等高」从约定变成结构。
        static let bar: CGFloat = 56
        /// 控制栏里单个按钮的点击区，HIG 下限。
