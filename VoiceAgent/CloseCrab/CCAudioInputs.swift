    ...
    final class CCAudioInputs {
        ...
        ///
        /// **「声道数 > 0 ＝ 输入设备」这条判据本身就错。**
        /// `MacBook Air Speakers`（`uid=BuiltInSpeakerDevice`）报 **4 个输入声道**。
        /// 不是查询失效、不是时序问题 —— 它在 CoreAudio 里确实声明了输入流
        /// （回声消除参考通道那类东西）。
        ///
        /// ⚠️ 精确措辞：四组采样里它出现了三次，**出现的那三次都是 4ch**。
        /// （第二组它整个没出现 —— 清单本身在抖，见下面「已知未解」。）
        /// 初稿写的是「每一次采样都报 4ch」，那是**把"它出现时"说成了"每一次"**，
        /// 被 tommy 拿 `grep -c` 核出来了。多出来的那层绝对化不影响结论，
        /// 但它会让后来人以为这台设备一直在清单里。
        ///
