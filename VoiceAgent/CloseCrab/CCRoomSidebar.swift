...
/// 宽屏的房间侧栏 —— 取代手机上那排横滚的小方块。
///
/// **Mac 和 iPad 共用这一份。** 2026-09-22 之前它整个包在 `#if os(macOS)` 里，
/// 名字也叫 `CCMacRoomSidebar`。去掉那层闸门时逐行看过，
/// 里面**没有一个 macOS 专属 API** —— `.listStyle(.sidebar)`、
/// `.navigationSplitViewColumnWidth`、`.onHover`、`.contextMenu`、`.help`
/// 在 iPadOS 上全都在，只是表现方式不同（右键 → 长按，悬停 → 指针设备）。
///
/// ⇒ 一般化：**`#if os(macOS)` 常常记录的是「当时只有 Mac 要用」，
/// 不是「只有 Mac 能用」。** 这两件事在代码里长得一模一样，
/// 而前者会在下一个平台进来时变成假的约束。
///
/// ## 为什么不是把方块行搬过来
///
...
///
/// 宽屏正好反过来 —— **竖向有的是空间，横向是拿来放内容的**。
/// 而且横向滚动在鼠标上本来就别扭（触控板还行，鼠标滚轮多数只有竖轴）。
///
/// ## 侧栏能做而方块行做不到的那件事
///
...
/// 恰恰是靠**大部分行是安静的**才跳得出来。
struct CCRoomSidebar: View {
    let rooms: CCRooms
    /// 「管理房间」抽屉的开关。**由 `CCShell` 持有** —— Mac 上 ⌘K 走的也是它，
            ...
            Button {
                ...
                HStack(spacing: CC.Space.tight) {
                    ...
                    Spacer()
                        ...
                        // 是没人发现它存在。
                        //
                        // ⚠️ **iPad 上不画这个。** ⌘K 是 `CCMacCommands` 注册的，
                        //    那整个文件是 macOS 专属 —— iPad 上按 ⌘K 什么都不会发生。
                        //    写一个不存在的快捷键，比不写更坏：它会让人以为
                        //    是自己按错了，然后再也不试第二次。
                        Text(verbatim: "⌘K")
...
/// 侧栏里的一行。
private struct CCRoomSidebarRow: View {
    ...
    /// 指针悬停。**有指针设备时这不是锦上添花** —— 一个鼠标划过去毫无反应的
    /// 列表，用起来的感觉是「死的」，而那个印象比布局不对更早形成。
    /// iPad 接了妙控板/鼠标同样有效；纯手指时它恒为 false，不影响任何东西。
    ...
    var body: some View {
        ...
        //
        // 右键菜单。**同一个修饰符在两个平台是两种手势** ——
        // Mac 上是右键，iPad 上系统自动映射成长按。一处管两端，不用另写。
        .contextMenu {
